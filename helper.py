...
try:
    ...
    print(
        "Warning: webdriver-manager not installed. Install with: pip install webdriver-manager"
    )

from board import create_literaki_board, BOARD_SIZE
...
CHROMEDRIVER_PATH = (
    "chromedriver"  # Fallback path if webdriver-manager fails or is not installed
)
...
logging.basicConfig(
    ...
    handlers=[
        logging.FileHandler(
            "literaki_helper.log", encoding="utf-8"
...
class PerformanceMetrics:
    ...
    def record_state_transition(self, from_state, to_state):
        transition_record = {
            ...
            "from": (
                from_state.value
                if isinstance(from_state, GameStateEnum)
                else from_state
            ),
        ...
        logging.info(
            f"State transition: {transition_record['from']} -> {transition_record['to']}"
        )
    ...
    try:
        ...
        try:
            if OCR_GPU:
                ...
                logging.info(
                    "  -> EasyOCR initialized with CPU support (GPU disabled by config)."
                )
        except Exception as e_gpu:
            logging.warning(
                f"  -> GPU initialization failed ({e_gpu}), falling back to CPU."
            )
            try:
                ocr_reader = easyocr.Reader(["pl"], gpu=False)
        ...
        if not os.path.exists("dictionary.txt"):
            logging.warning(
                "  -> Warning: dictionary.txt not found. Creating minimal dictionary."
            )
...
if (
    not validate_dependencies()
):  # Assuming validate_dependencies is defined elsewhere or simplified
...
def launch_browser_and_navigate():
    ...
    if WEBDRIVER_MANAGER_AVAILABLE:
        try:
            ...
            logging.warning(
                f"  -> webdriver-manager failed: {e}. Falling back to manual path if specified."
            )
    ...
    if driver_service is None and CHROMEDRIVER_PATH:
        ...
        logging.error(
            "  -> No ChromeDriver path specified and webdriver-manager failed or not available."
        )
        return None
    ...
    try:
        ...
        logging.info(
            "  -> Chrome launched. Please join a game manually in the browser."
        )
        return driver
    except Exception as e:
...
def find_board_in_screenshot(full_bgr):
    ...
    try:
        ...
        lower_red1 = np.array(
            [0, 100, 100]
        )  # Adjusted HSV ranges slightly for broader red/orange
        ...
        )  # Optional: helps connect broken contours
        ...
        if len(centroids) < 4:  # Need at least 4 markers for corners
            logging.debug(
                f"Not enough red markers found: {len(centroids)} (need at least 4 for board outline)"
            )
            return None
        ...
        if (
            board_candidate_width < 150 or board_candidate_height < 150
        ):  # Heuristic: board should be reasonably large
            logging.debug(
                f"Detected red markers span too small an area: W={board_candidate_width}, H={board_candidate_height}"
            )
            return None
        confidence_score += 10  # For significant span

        aspect_ratio = (
            board_candidate_width / board_candidate_height
            if board_candidate_height > 0
            else 0
        )
        if not (0.8 < aspect_ratio < 1.2):
            logging.debug(
                f"Red markers don't form a square (aspect ratio: {aspect_ratio:.2f})"
            )
            return None
        ...
        board_img_check = full_bgr[
            board_top : board_top + board_height, board_left : board_left + board_width
        ]
        if board_img_check.size == 0:
            return None  # Avoid error if rect is invalid
        gray_board_check = cv2.cvtColor(board_img_check, cv2.COLOR_BGR2GRAY)
        _, thresh_board_check = cv2.threshold(
            gray_board_check, 220, 255, cv2.THRESH_BINARY
        )  # Higher threshold for "very bright"
        active_pixels = cv2.countNonZero(thresh_board_check)
        if active_pixels > (
            board_width * board_height * 0.5
        ):  # Increased threshold: if >50% is very bright
            logging.debug(
                "Board area too bright, might not be active game state or wrong detection."
            )
            # return None # Commented out: this check might be too aggressive for empty boards
            confidence_score -= (
                10  # Penalize if too bright, but don't outright reject yet
            )
        else:
            confidence_score += 30

        logging.debug(
            f"Board detection proposal: Rect=({board_left},{board_top},{board_width},{board_height}), TileW/H=({tile_w:.1f},{tile_h:.1f}), Confidence={confidence_score}/100"
        )
        return (
            board_left,
            board_top,
...
def detect_waiting_screen(image_bgr: np.ndarray) -> bool:
    ...
    try:
        ...
        text_results = ocr_reader.readtext(
            ...
            x_ths=0.8,
            low_text=0.4,
        )  # Common EasyOCR tuning parameters
...
def ocr_board(tile_w, tile_h, board_rect, board_img_bgr):
    ...
    try:
        ...
        logging.info(
            f"OCR Debug: Board region (relative to cropped image: {bx},{by},{bw},{bh}), tile size ({tile_w:.1f},{tile_h:.1f})"
        )
        ...
        board_results_orig = ocr_reader.readtext(
            board_img, batch_size=16, workers=0 if OCR_GPU else 4
        )

        logging.info("OCR Debug: Running EasyOCR on preprocessed image...")
        board_results_proc = ocr_reader.readtext(
            sharpened_board, batch_size=16, workers=0 if OCR_GPU else 4
        )

        # Log all OCR results before filtering
        logging.info(
            f"OCR Debug: Original image detected {len(board_results_orig)} text regions:"
        )
        for i, (bbox, text, prob) in enumerate(board_results_orig):
            logging.info(
                f"  [{i}] Text: '{text}' | Confidence: {prob:.3f} | BBox: {bbox}"
            )

        logging.info(
            f"OCR Debug: Preprocessed image detected {len(board_results_proc)} text regions:"
        )
        ...
        board_results = (
            board_results_proc
            if len(board_results_proc) > len(board_results_orig)
            else board_results_orig
        )
        logging.info(
            f"OCR Debug: Using {'preprocessed' if board_results == board_results_proc else 'original'} image results"
        )
        ...
        for bbox, text, prob in board_results:
            ...
            if prob < OCR_CONFIDENCE_THRESHOLD:
                logging.debug(
                    f"  -> Rejected: confidence {prob:.3f} < {OCR_CONFIDENCE_THRESHOLD}"
                )
                rejected_letters += 1
                continue
            ...
            if len(txt) == 1 and txt in char_mapping:
                ...
                if mapped_char in TILE_DEFINITIONS:
                    ...
                    if 0 <= row < BOARD_SIZE and 0 <= col < BOARD_SIZE:
                        ...
                        accepted_letters += 1
                        logging.debug(
                            f"  -> Accepted: '{mapped_char}' at ({row},{col})"
                        )
                    else:
                        logging.debug(
                            f"  -> Rejected: position ({row},{col}) out of bounds"
                        )
                        rejected_letters += 1
                else:
                    logging.debug(
                        f"  -> Rejected: '{mapped_char}' not in TILE_DEFINITIONS"
                    )
                    rejected_letters += 1
            else:
        ...
        logging.info(
            f"OCR Debug: Board processing complete - Accepted: {accepted_letters}, Rejected: {rejected_letters}"
        )
...
def ocr_rack(rack_img_bgr: np.ndarray) -> list:
    ...
    try:
        ...
        rack_results = ocr_reader.readtext(
            sharpened_rack, batch_size=8, workers=0 if OCR_GPU else 2
        )
...
def update_gui_from_detection(board_letters, rack_letters):
    ...
    try:
                ...
                if letter:  # If a letter is detected for this square
                    if letter in TILE_DEFINITIONS:
                        ...
                        new_placed_letters[r][c] = {
                            ...
                            "tile_obj": {"letter": letter, "points": 0, "color": None},
                        }
        ...
        gui.player_rack.tiles = (
            new_rack_tiles  # Assuming player_rack has a 'tiles' attribute
        )
    ...
    while True:
        ...
        try:
            ...
            if full_screenshot_pil is None:
                logging.error(
                    "Failed to capture screenshot (pyautogui.screenshot() returned None)."
                )
            ...
            board_cropped_filename = os.path.join(
                SCREENSHOT_DIR, f"board_cropped_{timestamp}.png"
            )
            ...
            rack_cropped_filename = os.path.join(
                SCREENSHOT_DIR, f"rack_cropped_{timestamp}.png"
            )
            ...
            if gui:
                gui.display_message("Screenshot error. Retrying...", (200, 0, 0))
            time.sleep(REFRESH_INTERVAL)
            continue
        ...
        if detect_waiting_screen(
            board_cropped_bgr
        ):  # Use cropped board for waiting screen detection
            ...
            if (
                current_game_state != GameStateEnum.PLAYING
            ):  # Only re-calculate if not already playing
                ...
                board_rect_stable = (
                    ...
                    b_w_fixed,
                    b_h_fixed,
                )  # Relative to cropped image
                last_ocr_time = 0
                if gui:
                    gui.display_message("Board active. Assisting...", (0, 200, 0))

            if (
                new_state == GameStateEnum.PLAYING
                and board_rect_stable
                and (
                    time.time() - last_ocr_time >= REFRESH_INTERVAL
                    or last_ocr_time == 0
                )
            ):
                ...
                if gui:
                    if best_m:
                        ...
                        title_text = gui.font_title.render(
                            "LITERAKI HELPER - ACTIVE", True, (50, 50, 50)
                        )
...
if __name__ == "__main__":
    try:
        ...
        if driver is None:
            ...
            if gui:
                gui.display_message(
                    "Browser launch failed. Check console.", (200, 0, 0), duration=5
                )
            sys.exit(1)

        logging.info(
            f"Waiting {INITIAL_WAIT_TIME} seconds for page to load and user to join game..."
        )
        if gui:
            gui.display_message(
                f"Browser launched. Join game. Waiting {INITIAL_WAIT_TIME}s...",
                (0, 100, 200),
            )
