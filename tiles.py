...
TILE_DEFINITIONS = {
    ...
    "Z": {
        ...
        "count": 5,
        "color": BC_BLUE,
    },  # Assuming 'Z' is 1 point, count 5 (common in Polish Scrabble)
...
class PlayerRack:
    ...
    def remove_tile(self, letter_char_or_tile_obj):
        ...
        if isinstance(letter_char_or_tile_obj, str):
            ...
            isinstance(letter_char_or_tile_obj, dict)
            and "letter" in letter_char_or_tile_obj
        ):
            try:
...
if __name__ == "__main__":
    ...
    if len(player1_rack.get_tiles()) >= 1:
        ...
        print(
            f"Player 1 rack after playing: {[t['letter'] for t in player1_rack.get_tiles()]}"
        )
        player1_rack.replenish_tiles()
        print(
            f"Player 1 rack after replenishing: {[t['letter'] for t in player1_rack.get_tiles()]}"
        )
